...
def essc(X,K,gamma,eta,iterations):
    N, D = X.shape
    if (min(N, D - 1)) >= 3:
    ...
    for i in range(iterations):
        ...
        costF[i]=costFunction(X,u,V,V0,eta,gamma,w,m)
    u=np.nan_to_num(u)
    max=np.max(u,axis=0)
...
class ESSC:
    ...
    eta = 0
    isConverge = False
    def __init__(self,n_clusters=3,max_iter=20,gamma=10.0,eta=0.001):
        self.n_clusters=n_clusters
        self.max_iter=max_iter
        self.gamma=gamma
        self.eta=eta
    def fit(self,X,y=None):
        self.isConverge,self.best_labels,self.best_centers=essc(
