...
def ewkmeans(X,K,gamma,max_iter):
    n, m = X.shape
    costF = []
    ...
    for i in range(max_iter):
        ...
        c=costFunction(X,centroids,idx,lambdaa,gamma)
...
class EWKmeans:
    ...
    def fit(self,X):
        self.isConverge,self.best_labels,self.best_centers,self.cost=ewkmeans(
            X=X,K=self.n_cluster,gamma=self.gamma,max_iter=self.max_iter
        )
        return self
    ...
    def get_params(self):
        return self.isConverge,self.n_cluster,self.gamma
    def get_cost(self):
        return self.cost
